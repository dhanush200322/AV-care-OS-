...
export const SecurityTopNavbar: React.FC<{ onSearchClick: () => void; onAIToggle: () => void; scrolled?: boolean }> = ({ onSearchClick, onAIToggle, scrolled }) => {
  const { theme, toggleTheme } = useTheme();
  const { notifications, isEmergencyMode, toggleEmergencyMode } = useStore();
  const { wsConnected, incidents, alerts } = useSecurityStore();
  ...
  return (
    <motion.header animate={{ boxShadow: scrolled ? '0 8px 40px rgba(255,68,48,0.15)' : '0 4px 24px rgba(0,194,224,0.1)' }} className="sticky top-0 z-40 mx-4 mt-4 mb-2 rounded-2xl border border-[#00C2E0]/25 bg-[#0A1824]/85 backdrop-blur-xl px-4 py-3 flex flex-wrap items-center gap-3">
      ...
      <button type="button" className="relative p-2.5 rounded-xl border border-white/10">
        <Bell size={18} />
        {(criticalAlerts + notifications.filter((n) => !n.read).length) > 0 && (
          <span className="absolute -top-0.5 -right-0.5 min-w-4 h-4 rounded-full bg-[#FF4444] text-[9px] font-black text-white flex items-center justify-center animate-pulse">{criticalAlerts}</span>
        )}
      </button>
