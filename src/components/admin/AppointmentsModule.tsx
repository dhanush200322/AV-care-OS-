      ...
      <div className="flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-4 bg-white/5 p-4 rounded-3xl border border-white/10">
        <div className="flex-1 flex flex-col md:flex-row items-stretch gap-3">
          <div className="relative flex-1 group">
            ...
            <input 
              type="text"
              placeholder={t("Search patients, specialist name, ID...")}
              value={searchTerm}
          ...
          <div className="flex gap-2">
            ...
            >
              <option className="bg-slate-900 text-white" value="All">{t('All Appointments')}</option>
              <option className="bg-slate-900 text-white" value="Confirmed">{t('Confirmed')}</option>
              <option className="bg-slate-900 text-white" value="Pending">{t('Pending')}</option>
              <option className="bg-slate-900 text-white" value="Canceled">{t('Canceled')}</option>
            </select>
          </div>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-white/5">
                <th className="p-6 text-[10px] font-black uppercase tracking-[0.2em] text-white/30">{t('ID')}</th>
                <th className="p-6 text-[10px] font-black uppercase tracking-[0.2em] text-white/30">{t('Patient Name')}</th>
                <th className="p-6 text-[10px] font-black uppercase tracking-[0.2em] text-white/30">{t('Doctor Assignee')}</th>
                <th className="p-6 text-[10px] font-black uppercase tracking-[0.2em] text-white/30">{t('Specialty Unit')}</th>
                <th className="p-6 text-[10px] font-black uppercase tracking-[0.2em] text-white/30">{t('Date & Slot')}</th>
                <th className="p-6 text-[10px] font-black uppercase tracking-[0.2em] text-white/30">{t('Status')}</th>
                <th className="p-6 text-[10px] font-black uppercase tracking-[0.2em] text-white/30 text-right">{t('Action')}</th>
              </tr>
            </thead>
            <tbody>
                ...
                <tr key={apt.id} className="border-b border-white/5 hover:bg-white/[0.02] group transition-colors">
                  ...
                  <td className="p-6">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-xs font-bold text-white">{t(apt.patientName)}</span>
                    </div>
                  </td>
                  <td className="p-6">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-xs font-semibold text-white/80">{t(apt.doctorName)}</span>
                    </div>
                  </td>
                  <td className="p-6 text-xs text-white/40 tracking-wider font-semibold">{t(apt.specialty)}</td>
                  <td className="p-6">
                ...
                <tr>
                  <td colSpan={7} className="p-20 text-center">
                    <div className="flex flex-col items-center gap-4 opacity-20">
                      <Calendar size={48} />
                      <p className="text-xs font-black uppercase tracking-[0.2em]">{t('No Bookings Found')}</p>
                    </div>
                  </td>
                </tr>
              )}
        ...
        {isAddOpen && (
          <div className="fixed inset-0 z-[160] flex items-center justify-center p-4">
            ...
            >
              ...
              <h2 className="text-xl font-black text-white uppercase tracking-widest mb-6 border-b border-white/5 pb-3">{t('Book Slot')}</h2>
              
              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  <label className="text-[10px] font-black text-white/40 uppercase tracking-widest block mb-2">{t('Patient Select / Name')}</label>
                  <select
                    value={patientName}
                  ...
                  >
                    <option value="" className="text-slate-400 bg-slate-900">-- {t('Choose Patient')} --</option>
                    {patients.map(p => (
                       <option key={p.id} value={p.name} className="bg-slate-900 text-white">{t(p.name)}</option>
                    ))}
                    <option value="New Outpatient Intake" className="bg-slate-900 text-purple-400">{t('Add New Outpatient Intake')}</option>
                  </select>
                </div>

                <div>
                  <label className="text-[10px] font-black text-white/40 uppercase tracking-widest block mb-1">{t('Doctor / Medical Consultant')}</label>
                  <select
                    value={doctorName}
                    ...
                    className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white text-xs font-bold focus:outline-none focus:border-purple-500/50"
                  >
                    <option value="" className="text-slate-400 bg-slate-900">-- {t('Select Officer')} --</option>
                    {doctors.map(d => (
                       <option key={d.id} value={d.name} className="bg-slate-900 text-white">{t(d.name)} ({t(d.role)})</option>
                    ))}
                  </select>
                </div>

                <div>
                  <label className="text-[10px] font-black text-white/40 uppercase tracking-widest block mb-1">{t('Consultation Category')}</label>
                  <select
                    value={specialty}
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="text-[10px] font-black text-white/40 uppercase tracking-widest block mb-1">{t('Date')}</label>
                    <input 
                      type="date"
                      required
                      value={date}
                  ...
                  <div>
                    <label className="text-[10px] font-black text-white/40 uppercase tracking-widest block mb-1">{t('Slot Time')}</label>
                    <input 
                      type="text"
                      required
                      value={time}
                      onChange={(e) => setTime(e.target.value)}
                      placeholder={t("e.g. 11:30 AM")}
                      className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white text-xs font-bold focus:outline-none focus:border-purple-500/50"
                    />
                  </div>
                </div>
                ...
                >
                  {t('Confirm Scheduled Appointment')}
                </button>
              </form>
            </motion.div>
          </div>
