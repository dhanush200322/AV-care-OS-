      ...
      <div className="flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-4 bg-white/5 p-4 rounded-3xl border border-white/10">
        <div className="flex-1 flex flex-col md:flex-row items-stretch gap-3">
          <div className="relative flex-1 group">
            ...
            <input 
              type="text"
              placeholder="Search physicians, specialties or status..."
              value={searchTerm}
          ...
          <div className="flex flex-wrap gap-2">
            ...
            >
              {specialties.map(spec => (
                <option key={spec} className="bg-slate-900 text-white" value={spec}>{spec} Department</option>
              ))}
            </select>
            ...
            <select
              ...
              className="bg-slate-950/40 border border-white/5 rounded-xl px-4 py-2.5 text-xs text-white font-bold tracking-wide focus:outline-none focus:border-purple-500/50"
            >
              <option className="bg-slate-900 text-white" value="All">All Status</option>
              <option className="bg-slate-900 text-white" value="On Duty">🟢 On Duty</option>
              <option className="bg-slate-900 text-white" value="Off Duty">🔴 Off Duty</option>
              <option className="bg-slate-900 text-white" value="Emergency">🚨 Emergency Only</option>
            </select>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          ...
          >
            ...
            <div className="flex justify-between items-start">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-sm font-black text-white group-hover:text-purple-300 transition-colors uppercase tracking-wider">{doc.name}</h3>
                  <p className="text-[10px] font-bold text-white/40 uppercase tracking-widest">{doc.role}</p>
                </div>
              </div>
            ...
            <div className="my-3 space-y-1 bg-white/[0.02] border border-white/5 rounded-2xl p-3">
              <div className="flex justify-between text-[10px] font-bold uppercase tracking-wider text-white/30">
                ...
                <span className="text-cyan-400">{doc.department}</span>
              </div>
              <div className="flex justify-between text-[10px] font-bold uppercase tracking-wider text-white/30">
        ...
        {filteredDoctors.length === 0 && (
          <div className="col-span-full py-16 text-center border border-dashed border-white/5 rounded-3xl flex flex-col items-center justify-center opacity-40 gap-4">
             ...
             <div>
               <p className="text-xs font-black uppercase tracking-[0.2em]">No Medical Officers Found</p>
               <p className="text-[10px] text-white/40 mt-1">Adjust search options or register a new doctor.</p>
             </div>
          </div>
        )}
      </div>
        ...
        {isAddOpen && (
          <div className="fixed inset-0 z-[160] flex items-center justify-center p-4">
            ...
            >
              ...
              <h2 className="text-xl font-black text-white uppercase tracking-widest mb-6 border-b border-white/5 pb-3">Register Doctor</h2>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                  <label className="text-[10px] font-black text-white/40 uppercase tracking-widest block mb-2">Roster Position</label>
                  <input 
                    type="text"
                    required
                    value={role}
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="text-[10px] font-black text-white/40 uppercase tracking-widest block mb-2">Specialty Ward</label>
                    <select
                      value={department}
                    ...
                    >
                      ...
                      <option className="bg-slate-900 text-white" value="Orthopedics">Orthopedics</option>
                      <option className="bg-slate-900 text-white" value="General Medicine">General Medicine</option>
                    </select>
                  </div>

                  <div>
                    <label className="text-[10px] font-black text-white/40 uppercase tracking-widest block mb-2">Operation Status</label>
                    <select
                      value={status}
                    ...
                    >
                      <option className="bg-slate-900 text-white" value="On Duty">On Duty</option>
                      <option className="bg-slate-900 text-white" value="Off Duty">Off Duty</option>
                      <option className="bg-slate-900 text-white" value="Emergency">Emergency</option>
                    </select>
                  </div>
                </div>

                <div>
                  <label className="text-[10px] font-black text-white/40 uppercase tracking-widest block mb-2">Phone / Contact direct</label>
                  <input 
                    type="text"
                    required
                    value={contact}
